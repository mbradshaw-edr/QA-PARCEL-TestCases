...
public class Assemble_Tab {
	...
	private void Upload_Image() throws Exception{
		...
		BrowserName = cap.getBrowserName().toLowerCase();	
		if (BrowserName.equals("chrome")) {
			Runtime.getRuntime().exec("src\\Test_Data\\Image upload for Chrome.exe");
		} else if (BrowserName.equals("internet explorer")) {
			Runtime.getRuntime().exec("src\\Test_Data\\Image upload for IE.exe");
		}
		else{
			Runtime.getRuntime().exec("src\\Test_Data\\Image upload for Firefox.exe");
		}
		Thread.sleep(15000);
	}

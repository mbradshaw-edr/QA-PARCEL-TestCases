...
public class Header_Menus_Home_Page {
	...
	@FindBy(how = How.XPATH, using = "//a[text()='Setup']")
	WebElement Setup_Tab;

	@FindBy(how = How.XPATH, using = "//a[text()='Appendices']")
	WebElement Appendices_Tab;

	@FindBy(how = How.XPATH, using = "//a[text()='Sign']")
	WebElement Sign_Tab;

	@FindBy(how = How.XPATH, using = "//a[text()='Assemble']")
	...
	public void highlight_Project_Dropdown_Options() throws Exception {
		...
		for (int u = 1; u <= 2; u++) {
			...
			for (int i = 1; i <= 3; i++) {
				WebElement Project_Options = driver.findElement(
						By.xpath("html/body/div[1]/div/nav/div/ul/li[4]/ul/li/ul[" + u + "]/li[" + i + "]/a"));
	...
	public void highlight_Company_Dropdown_Options() throws Exception {
		...
		for (int u = 1; u <= 3; u++) {
			...
			for (int i = 1; i <= 4; i++) {
				...
				Thread.sleep(1000);
				WebElement Company_Options = driver
						.findElement(By.xpath("//*[@id='CompanyMenu']/li/ul[" + u + "]/li[" + i + "]/a"));
	...
	public void highlight_Username_Dropdown_Options() throws Exception {
		...
		for (int i = 1; i <= 4; i++) {
			...
			WebElement Username_Options = driver
					.findElement(By.xpath("html/body/div[1]/div/nav/div/ul/li[8]/ul/li/ul/li[" + i + "]/a"));
			...
			if (Account_Setting_Option.equals(Username_Options)) {
				...
				System.out.println(
						"Page Header present on Account Setting Page : " + Account_Setting_PageHeader.getText());
				Home_tab.click();
				Thread.sleep(10000);
				...
				try {
					...
					System.out.println("Page Header Present on Administrative Tools Page: "
							+ Administrative_Tools_PageHeader.getText());
				} catch (Exception e) {
	...
	public void Open_Report_In_Simplified(String Recently_Created_SiteId) throws Exception {
		...
		try {
			...
			Search_SiteID.sendKeys(excel.Site_Id("Site ID", 1, 0));
		}
	...
	public void Open_SiteID_In_Simplified() throws Exception {
		...
		SiteId_Field.sendKeys(excel.Site_Id("Site ID", 1, 0));
	...
	public void Check_Page_Headers() throws Exception {
		...
		System.out.println("Page Header on Order placed on EDR Website page: "
				+ Order_Placed_On_EDR_Website_Page_Header.getText());
		Home_tab.click();
		driver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
		Project_Dropdown.click();
		Thread.sleep(1000);
		...
		System.out.println("Page Header on Recent Portfolio Deliverables page: "
				+ Recent_Portfolio_Deliverables_Page_Header.getText());
		Home_tab.click();
		driver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
		Bids_Dropdown.click();
		Thread.sleep(8000);

...
public class Comments_Subtab {
			...
			if (D == 1) {
				for (int A = 1; A <= 2; A++) {
					WebElement element = driver.findElement(By.xpath(
							"//*[@id='contentappendicescomments-tab']/div[1]/div[2]/div[" + D + "]/div/a[" + A + "]"));
			...
			if (D == 2) {
				for (int A = 1; A <= 3; A++) {
					WebElement element = driver
							.findElement(By.xpath("//*[@id='contentappendicescomments-tab']/div[1]/div[2]/div[" + D
									+ "]/div/a[" + A + "]/span"));
					HighlightElement(driver, element);
					Thread.sleep(2000);
	...
	public void Add_Comments_And_Address() throws Exception {
		...
		for (WebElement A : All_Sections) {
			if (Section_No == 3 || Section_No == 4) {
				...
				List<WebElement> All_Edit_Comments = driver.findElements(By.xpath(
						"//*[@id='contentappendicescomments-tab']/div[2]/div[" + Section_No + "]/div/div/input[2]"));
					...
					try {
						if (EC.isDisplayed()) {
							...
							if (Counter == 1) {
								...
								EC.click();
								Thread.sleep(3000);
								List<WebElement> All_Edit_Textarea = driver
										.findElements(By.xpath("//*[@id='contentappendicescomments-tab']/div[2]/div["
												+ Section_No + "]/div/div/div[2]/textarea"));
								for (WebElement ET : All_Edit_Textarea) {
									try {
										if (ET.isDisplayed()) {
											...
											List<WebElement> All_Edit_Save = driver.findElements(
													By.xpath("//*[@id='contentappendicescomments-tab']/div[2]/div["
															+ Section_No + "]/div/div/div[2]/div/input[1]"));
				...
				List<WebElement> All_Address = driver.findElements(By.xpath(
						"//*[@id='contentappendicescomments-tab']/div[2]/div[" + Section_No + "]/div/div/input[3]"));
					...
					try {
						if (AA.isDisplayed()) {
							...
							if (Address_Counter == 1) {
								...
								AA.click();
								Thread.sleep(3000);
								List<WebElement> All_Address_Textarea = driver
										.findElements(By.xpath("//*[@id='contentappendicescomments-tab']/div[2]/div["
												+ Section_No + "]/div/div/div[3]/div/textarea"));
								for (WebElement AAT : All_Address_Textarea) {
									try {
										if (AAT.isDisplayed()) {
											...
											Thread.sleep(3000);
											List<WebElement> All_Address_Save = driver.findElements(
													By.xpath("//*[@id='contentappendicescomments-tab']/div[2]/div["
															+ Section_No + "]/div/div/div[3]/div/div/input[1]"));
	...
	public void Check_Section_Dropdown() throws Exception {
		...
		for (int A = 1; A <= Section_Dropdown_Options_Array.size(); A++) {
			...
			WebElement All_Dropdown_Options = driver
					.findElement(By.xpath("//*[@id='select2-drop']/ul/li[" + A + "]/div"));

			if ((All_Dropdown_Options.getText()).equalsIgnoreCase("Current Section")) {
				All_Dropdown_Options.click();
				Thread.sleep(3000);
				List<WebElement> All_Sections_of_Left_Panel = driver
						.findElements(By.xpath("//*[@id='contentappendicescomments-tab']/div[2]/div/h5"));
				...
				System.out.println("Section displaying after select Current Section Unaddressed Comments option: "
						+ Current_Section_Unaddressed_Comments);
				...
				System.out.println("Section displaying after select All Sections Unaddressed Comments option: "
						+ All_Sections_Unaddressed_Comments);
				JavascriptExecutor JJ = (JavascriptExecutor) driver;
				JJ.executeScript("arguments[0].scrollIntoView(true);", Section_Dropdown);
				Thread.sleep(3000);
			}
	...
	private void Check_Checkboxes() throws Exception {
		List<WebElement> All_Checkboxes = driver
				.findElements(By.xpath("//*[@id='contentappendicescomments-tab']/div[2]/div/div/div/input[1]"));
